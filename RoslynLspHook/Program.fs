...
// event as a JSON payload on stdin; `Logic.hook` reads that payload, infers the
// event from its shape, and drives the state machine. There are NO argv verbs any
// more: the broker daemon is now a separate executable (`RoslynLsp`) that the state
// machine spawns, so this entry point only ever runs the hook itself and can never
...
let main _argv =
    ...
    try
        Interpreter.run (Logic.hook Config.mkConfig)
    with _ ->
        ()
